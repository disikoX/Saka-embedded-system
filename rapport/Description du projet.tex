...
\section{Moyen nécessaire à la réalisation du projet}
	...
	Pour réaliser ce projet, on aura besoin des éléments suivants :
		\begin{itemize}
			...
			\item \textit{2 capteurs de poids} permettant de mesurer la quantité de nourriture distribuée ainsi que la quantité restante de nourriture dans le réservoir.
			\item \textit{Des bois et des vis} pour assembler le distributeur.
			\item \textit{2 LED} pour indiquer l'état du distributeur (distribution en cours, niveau bas de nourriture).
		\end{itemize}
	\subsection{Moyens Logiciels}
	Pour le développement du logiciel, les outils suivants seront utilisés :
		\begin{itemize}
			...
			\item \textit{Serveur Websocket (Node.js)} : pour gérer les requêtes entre l'ESP32 et la base de données.
			...
			\item \textit{TexMaker} : pour la rédaction du rapport de projet.
		\end{itemize}
